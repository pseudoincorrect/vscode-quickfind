...
export class RegexSearchProvider {
  ...
  private showSearchWebview(
    ...
    this.currentWebviewPanel = new SearchWebviewPanel(
      ...
      (result: SearchResult, isVerticalLayout: boolean) =>
        this.loadContextForResult(result, isVerticalLayout), // Add context loading callback
      this.searchService,
  ...
  private async performSearch(
    ...
    try {
      if (searchType === "file") {
        ...
        return await this.searchService.searchWithQuery(
          this.currentSearchPath,
          query,
          true,
        );
  ...
  private async loadContextForResult(
    ...
    return await this.searchService.loadContextForResult(
      result,
      isVerticalLayout,
    );
  }
