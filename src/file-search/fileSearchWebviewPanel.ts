...
export class FileSearchWebviewPanel {
  ...
  constructor(
    ...
    const configChangeListener = vscode.workspace.onDidChangeConfiguration(
      (event) => {
        if (
          event.affectsConfiguration("quickFind.maxResults") ||
          event.affectsConfiguration("quickFind.accentColor")
        ) {
  ...
  private setupEventHandlers() {
    ...
    const windowStateChangeListener = vscode.window.onDidChangeWindowState(
      (state) => {
