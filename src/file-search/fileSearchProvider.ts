...
export class FileSearchProvider {
  ...
  async searchFiles() {
    ...
    try {
      ...
      const initialResults = await this.searchService.searchFiles(
  ...
  private showSearchWebview(results: FileSearchResult[]) {
    ...
    this.currentWebviewPanel = new FileSearchWebviewPanel(
      ...
      (result: FileSearchResult, isVerticalLayout: boolean) =>
        this.loadContextForResult(result, isVerticalLayout),
      this.searchService,
      this.currentSearchPath,
      currentViewColumn,
    );
  ...
  private async performSearch(query: string): Promise<FileSearchResult[]> {
    ...
    try {
      return await this.searchService.searchFiles(
        this.currentSearchPath,
        query,
      );
    } catch (error) {
