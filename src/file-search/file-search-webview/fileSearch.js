...
function updateResultsList() {
    ...
    let html = results.slice(0, resultsToShow).map((result, index) => {
        ...
        const highlightedName = highlightSearchTerm(escapeHtml(fileName), currentSearchQuery);
        const highlightedPath = highlightSearchTerm(escapeHtml(relativePath), currentSearchQuery);
...
function updateContext() {
    if (results.length === 0 || !results[selectedIndex]) {
        contextPanel.innerHTML = '<div class="no-selection">Select a file to see context</div>';
        return;
    }
    ...
    if (result.context.length === 0) {
        contextPanel.innerHTML = '<div class="no-context">No content to display</div>';
        return;
    }
...
function getFileTypeClass(fileName, isDirectory) {
